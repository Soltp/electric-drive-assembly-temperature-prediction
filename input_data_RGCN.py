...
def load_water_data( ):
    Te_adj = pd.read_csv(r'../data/TNT_adj.csv',header=None)
    adj = np.mat(Te_adj)
    Te_tem = pd.read_csv(r'../data/measures.csv')
    return Te_tem, adj


def preprocess_data(data1,data2, All_cols,target_cols,test_profiles1,train_profiles,seq_len, pre_len):
    ...
    for i, (pid, x_train) in enumerate(data1.loc[data1.profile_id.isin(train_profiles), All_cols + ['profile_id']].groupby('profile_id')):
        ...
        datax01=np.delete(datax0, end-1, axis=1)
        for j in range(len(datax01) - seq_len - pre_len):
            a = datax01[j: j + seq_len + pre_len]
            trainX.append(a[0: seq_len])
    for i1, (pid, x_train1) in enumerate(data2.loc[data2.profile_id.isin(train_profiles),target_cols + ['profile_id']].groupby('profile_id')):
        end = x_train1.shape[1]
        datax1 = np.mat(x_train1, dtype=np.float32)
        datax11=np.delete(datax1, end-1, axis=1)
    ...
    for i2, (pid1, y_train) in enumerate(data1.loc[data1.profile_id.isin(test_profiles1), All_cols + ['profile_id']].groupby('profile_id')):
        ...
        datay01 = np.delete(datay0, end1-1, axis=1)
        for j2 in range(len(datay01) - seq_len - pre_len):
            b2 = datay01[j2: j2 + seq_len + pre_len]
            testX.append(b2[0: seq_len])
    for i3, (pid1, y_train1) in enumerate(data2.loc[data2.profile_id.isin(test_profiles1),target_cols + ['profile_id']].groupby('profile_id')):
        end1 = y_train1.shape[1]
        datay1 = np.mat(y_train1, dtype=np.float32)
        datay11 = np.delete(datay1, end1-1, axis=1)
