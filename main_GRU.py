...
from input_data_GRU import preprocess_data,load_water_data, ols_filter #002
...
flags.DEFINE_integer('gru_units', 16, 'hidden units of gru.')
...
flags.DEFINE_string('dataset', 'water', ' water or oil')
flags.DEFINE_string('model_name', 'gru', 'frgcn or tgcn or gru')
...
data01=df
Fill_features= ['pm', 'stator_tooth', 'stator_yoke', 'stator_winding','i_d','i_q']
target_cols = ['pm', 'stator_tooth', 'stator_yoke', 'stator_winding']
target_cols1= ['pm', 'stator_tooth', 'stator_yoke', 'stator_winding']
All_cols = ['pm', 'stator_tooth','stator_yoke','stator_winding']
...
df = pd.concat([df,
                pd.concat([ols_filter(p,spans) for p in p_df_list], ignore_index=True)],
               axis=1).dropna().reset_index(drop=True)
x_cols =[x for x in df.columns.tolist() if x not in Fill_features + [PROFILE_ID_COL]]
y_cols = target_cols

OLSpre_profiles= [13,31,18,30,29,8,20,15,14,16,36,5,32,21,7,3,9,27,11,24,74,75,41,68,50,53,49,81,60, 65, 72]#17,12,2,4,19,26,6,10,13,
...
y_train = trainset1.loc[:, y_cols]
x_test = pretest.loc[:, x_cols]
y_test = pretest.loc[:, y_cols]
...
trainX, trainY, testX, testY = preprocess_data(data1,data2,All_cols,test_profiles1,train_profiles,seq_len, pre_len)
...
path1 = '%s_%s_lr%s_delta%s_loss%r_batch%r_unit%r_seq%r_pre%r_epoch%r'%(model_name,data_name,lr,delta1,lambda_loss,batch_size,gru_units,seq_len,pre_len,training_epoch)
...
for epoch in range(training_epoch):
    for m in range(totalbatch):
        ...
        _, loss1, rmse1, train_output = sess.run([optimizer, loss, error, y_pred],feed_dict = {inputs:mini_batch, labels:mini_label})
...
print(time_end-time_start,'s',file=path2)
...
var.to_csv(path+'/min_error_result.csv',index = False,header = False)  # 将数据指标写入csv文件
...
var2.to_csv(path+'/min_RMSE_result.csv',index = False,header = False)  # 将数据指标写入csv文件
